...
where z̄ᵢ,ₗ,ⱼ is the average activation over response tokens of neuron *j* in layer *ℓ* for example *i*, and ‖z̄ᵢ,ₗ‖₂ is the L2 norm of the activation vector of layer *ℓ*. This normalization converts activations into relative contributions, comparable across layers and models regardless of activation magnitude scale. The final feature vector *X*ᵢ ∈ ℝᴺ concatenates the CETT scores of all layers and neurons, where *N* is the total number of FFN neurons in the model.
